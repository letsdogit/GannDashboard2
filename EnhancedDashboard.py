# gann_dashboard_enhanced.py
"""
GANN Pro — Enhanced Dashboard with Advanced Features
Run: streamlit run gann_dashboard_enhanced.py
NEW FEATURES: Visual indicators, Performance metrics, Fibonacci levels, 
              Comparison charts, Backtesting, CSV export, Day-of-week analysis
"""
...
# App config + CSS
# ---------------------------
st.set_page_config(page_title="GANN Pro — Enhanced", layout="wide", page_icon="📈")
st.markdown("""
<style>
...
st.markdown("<h2>📈 GANN Pro — Enhanced Dashboard</h2>", unsafe_allow_html=True)
st.markdown("<div class='small'>Advanced features: Performance analytics, Fibonacci levels, Backtesting, Market comparison & more!</div>", unsafe_allow_html=True)
st.write("")
...
def color_change(val):
    """Add emoji indicators for positive/negative changes"""
    try:
...
# Tabs
# ---------------------------
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📊 Overview","🎯 GANN Signals","📈 Charts","🔧 Tools","🔄 Compare","📁 Exports"])
...
with tab1:
    ...
    for m, df in market_data.items():
        if df is None or df.empty:
            ...
            last = df.iloc[-1]
            change_str = color_change(last.get('Return_Pct', np.nan))
            snapshot_rows.append({
                'Market': m, 
                'Latest Close': safe_fmt(last.get('Close', np.nan)), 
                '1d Change': change_str,
                'Status': '✓ Active'
            })
...
with tab2:
    ...
    if signals_df.empty:
        ...
        df_view['Change_Display'] = df_view['Change_Pct'].apply(color_change)
        
        # Display dataframe
        display_cols = ['GANN_Date', 'GANN_Type', 'Source', 'Market_Date', 'Close', 'Change_Display', 'MoveTag']
        st.dataframe(df_view[display_cols], use_container_width=True, height=400)
        
        if not df_view.empty:
            # Performance metrics
            st.markdown("### 📊 GANN Performance Analysis")
            ...
            big_moves = len(df_view[abs(df_view['Change_Pct']) > 2])
            
            avg_win = safe_mean(df_view[df_view['Change_Pct'] > 0]['Change_Pct'])
            avg_loss = safe_mean(df_view[df_view['Change_Pct'] < 0]['Change_Pct'])
            win_loss_ratio = abs(avg_win / avg_loss) if not math.isnan(avg_loss) and avg_loss != 0 else 0
            
            col1.metric("Total Signals", len(df_view))
            col2.metric("Big Moves (>2%)", big_moves)
            col3.metric("Avg Win", safe_fmt(avg_win, "{:.2f}%"))
            col4.metric("Avg Loss", safe_fmt(avg_loss, "{:.2f}%"))
            col5.metric("Win/Loss Ratio", safe_fmt(win_loss_ratio, "{:.2f}"))
            ...
            df_view_copy['DayOfWeek'] = pd.to_datetime(df_view_copy['GANN_Date']).dt.day_name()
            day_avg = df_view_copy.groupby('DayOfWeek')['Change_Pct'].mean().reindex(
            ...
            fig_heatmap = go.Figure(data=go.Bar(
                x=day_avg.index,
                y=day_avg.values,
                marker_color=['green' if x > 0 else 'red' for x in day_avg.values],
                text=[f"{x:.2f}%" for x in day_avg.values],
                textposition='auto'
            ))
            fig_heatmap.update_layout(
                title="Average Move by Day of Week",
                template='plotly_dark',
                height=300,
                xaxis_title="Day",
            ...
            with col2:
                if st.button("▶️ Run Backtest"):
                    ...
                    col2.metric("Final Capital", f"${capital:,.0f}")
                    col3.metric("Total Return", f"{total_return:.2f}%", delta=f"{total_return:.2f}%")
...
with tab4:
    ...
    if not ok_markets:
        ...
        if dfm.empty:
            ...
            high_52w = dfm['High'].rolling(252, min_periods=1).max().iloc[-1]
            low_52w = dfm['Low'].rolling(252, min_periods=1).min().iloc[-1]
            
            fib_levels = fibonacci_levels(high_52w, low_52w)
            fib_df = pd.DataFrame(list(fib_levels.items()), columns=['Level', 'Price'])
            fib_df['Price'] = fib_df['Price'].apply(lambda x: safe_fmt(x, '{:.2f}'))
            fib_df['Current'] = fib_df['Price'].apply(
                lambda x: '◄ Current' if abs(float(x.replace(',','')) - last_price) < (high_52w - low_52w) * 0.05 else ''
            )
            ...
            if 'VolSpike' in vdf.columns:
                ...
                if not spikes.empty:
                    ...
                    spike_display['Change'] = spike_display['Return_Pct'].apply(color_change)
                    st.dataframe(spike_display[['Date', 'Close', 'Volume', 'Change']], use_container_width=True)
                else:
...
with tab5:
    ...
    if len(ok_markets) >= 2:
        compare_markets = st.multiselect("Select markets to compare (2-5)", ok_markets, default=ok_markets[:min(3, len(ok_markets))])
        
        if len(compare_markets) >= 2:
            fig_compare = go.Figure()
            
            for market in compare_markets:
                df_norm = market_data[market].copy()
                # Normalize to base 100
                df_norm['Normalized'] = (df_norm['Close'] / df_norm['Close'].iloc[0]) * 100
            ...
            for market in compare_markets:
                ...
                perf_rows.append({
                    'Market': market,
                    'Start Price': safe_fmt(first_close),
                    'Current Price': safe_fmt(last_close),
                    'Total Return %': safe_fmt(total_return, '{:.2f}%'),
                    'Indicator': color_change(total_return)
                })
        ...
        st.info("Load at least 2 markets to use comparison features")
        ...
        if edf.empty:
            ...
            with col1:
                ...
                if st.button("📥 Download CSV"):
                    ...
                    download_link = f'<a href="data:file/csv;base64,{b64}" download="gann_signals_{datetime.now().strftime("%Y%m%d")}.csv" class="download-btn">📥 Download CSV</a>'
                ...
                if enable_excel:
                    if st.button("📥 Download Excel", key="excel_btn"):
                        try:
                            ...
                            download_link = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="GANN_Export_{export_market}_{datetime.now().strftime("%Y%m%d")}.xlsx" class="download-btn">📥 Download Excel</a>'
                            st.markdown(download_link, unsafe_allow_html=True)
                ...
                if enable_pdf:
                    if st.button("📥 Download PDF", key="pdf_btn"):
                        try:
                            ...
                            pdf.cell(0, 5, "This report is for educational purposes only. Not financial advice.", ln=True)
                            
                            pdf_bytes = pdf.output(dest='S').encode('latin-1')
                            b64 = base64.b64encode(pdf_bytes).decode()
                            
                            download_link = f'<a href="data:application/pdf;base64,{b64}" download="GANN_Report_{export_market}_{datetime.now().strftime("%Y%m%d")}.pdf" class="download-btn">📄 Download PDF</a>'
                            st.markdown(download_link, unsafe_allow_html=True)
...
<div style='text-align: center; color: var(--muted); font-size: 12px; padding: 20px 0;'>
    <p><strong>© 2025 GANN Pro Enhanced Dashboard</strong> • Advanced Technical Analysis Platform</p>
    <p>⚠️ Disclaimer: Educational and informational purposes only. Not financial advice.</p>
